...
describe Place do
  ...
  describe 'when asked to find by ascii_name or alternate_names' do
    it 'should correctly escape the search term' do
      ...
      Place.should_receive(:find_all_by_ascii_name).with("Her Majesty\'s Opposition\'s", {:order => "feature_class"}).and_return([mock_model(Place)])
    ...
    it 'should find a matching place if the place name is stored without the apostrophe' do
      term = "King's Lynn"
      Place.should_receive(:find_all_by_ascii_name).with("King\'s Lynn", {:order => "feature_class"}).and_return([])
      Place.should_receive(:find_all_by_ascii_name).with("Kings Lynn", {:order => "feature_class"}).and_return([mock_model(Place)])
    ...
    it 'should return a single result where there is a direct name match' do
      ...
      Place.should_receive(:find).with(:all, :order => "feature_class", :conditions => {:name => 'Big Ben'}).and_return([place])
    ...
    it 'should return a single result where there is a direct ascii_name match' do
      term = "Big Ben"
      place = mock_model(Place)
      Place.should_receive(:find).with(:all, :order => "feature_class", :conditions => {:name => 'Big Ben'}).and_return([])
      Place.should_receive(:find).with(:all, :order => "feature_class", :conditions => {:ascii_name => 'Big Ben'}).and_return([place])
      Place.should_receive(:find).with(:all, :order => "feature_class", :conditions => "alternate_names = 'Big Ben' or alternate_names like 'Big Ben,%' or alternate_names like '%,Big Ben,%' or alternate_names like '%,Big Ben'").and_return []
    ...
    it 'should return a single result where there is one match within alternate_names' do
      ...
      Place.should_receive(:find).with(:all, :order => "feature_class", :conditions => {:name => 'Essex'}).and_return([])
      Place.should_receive(:find).with(:all, :order => "feature_class", :conditions => {:ascii_name => 'Essex'}).and_return([])
     ...
     it 'should return an array of results - where there are matches within name and alternate_name' do
       ...
       Place.should_receive(:find).with(:all, :order => "feature_class", :conditions => {:name => 'Bedford'}).and_return([])
       Place.should_receive(:find).with(:all, :order => "feature_class", :conditions => {:ascii_name => 'Bedford'}).and_return([place1])
